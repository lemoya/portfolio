(function() {
  ...
  stars.forEach(star => {
    star.addEventListener('click', () => {
      ...
      ratingOutput.innerHTML = `Rating: ${rating} stars`;

      // Highlight the stars up to the selected rating
      stars.forEach(star => {
        if (star.getAttribute('data-rating') <= rating) {
          star.style.color = '#f39c12';  // Selected stars
        } else {
          star.style.color = '#ccc';  // Unselected stars
        }
      });
    });
  });
  ...
  commentForm.addEventListener('submit', (event) => {
    ...
    if (commentText) {
      ...
      commentDiv.innerHTML = `<p><strong>Your comment:</strong> ${commentText}</p>`;

      // Append the new comment to the comment output section
...
document.querySelector('.php-email-form').addEventListener('submit', function (e) {
  ...
  .then(data => {
      if (data.success) {
          ...
          alert(data.error || 'There was an issue sending the message.');
      }
  })
  .catch(error => {
